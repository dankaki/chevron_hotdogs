...
for store in range(1000, 5000, 1000):
    for bucket in range(1, 5):
        print('Enhancing the weights for store {0}, bucket {1}'.format(store, bucket))
        ...
        optimizer = torch.optim.Adam(sales_net.parameters(), lr = 0.005)
        ...
        torch.save(sales_net, f = 'enhance1/s{0}b{1}'.format(store, bucket))